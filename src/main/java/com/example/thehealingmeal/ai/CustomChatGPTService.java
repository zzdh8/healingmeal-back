...
public class CustomChatGPTService {
    ...
    public void saveResponse(AiResDto aiResDto, Meals meals) throws IllegalArgumentException{
        if (aiResDto.getAnswer().isEmpty() && aiResDto.getAnswer().isBlank()){
            throw new IllegalArgumentException("GPT Response is Empty");
        }
        GPTResponse gptResponse = GPTResponse.builder()
                .gptAnswer(aiResDto.getAnswer())
                .meals(meals)
                .build();
